        ...
        if isinstance(parsed, dict):
            ...
            if isinstance(params, dict):
                ...
                if fhir_data:
                    ...
                    logger.info(
                        "FHIR_PATIENT_FOUND value=%s", fhir_data.get("patientId", "[EMPTY]")
                    )
                else:
                    logger.info(
                        "FHIR_NOT_FOUND_IN_PAYLOAD keys_checked=params.metadata,message.metadata"
                    )
        ...
        if "application/json" in content_type:
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("kind") == "task":
                    ...
                    logger.info(
                        "response_reshaped_to_po_a2a_json task_id=%s state=%s",
                        task.get("id"),
                        task["status"]["state"],
                    )
