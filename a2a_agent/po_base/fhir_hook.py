...
def _safe_correlation_ids(callback_context, llm_request) -> dict:
    return {
        "task_id": _first_non_empty(
            getattr(llm_request, "task_id", None), getattr(callback_context, "task_id", None)
        ),
        "context_id": _first_non_empty(
            getattr(llm_request, "context_id", None), getattr(callback_context, "context_id", None)
        ),
        "message_id": _first_non_empty(
            getattr(llm_request, "message_id", None), getattr(callback_context, "message_id", None)
        ),
    }
...
def _extract_metadata_sources(callback_context, llm_request) -> list:
    ...
    a2a_metadata = (
        custom_metadata.get("a2a_metadata") if isinstance(custom_metadata, dict) else None
    )
