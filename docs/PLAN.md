...
- **Payer criteria accuracy + IP** → paraphrase (don't copy) Cigna/Aetna criteria; cite canonical source policy URL in each JSON; cross-check against public ACR Appropriateness Criteria; ideally one clinician review before Week 3. **Mitigation workflow** (post-PR #6): every payer criteria entry follows a two-stage research-then-verify pass — automated extraction may use mirrors / PDF caches when canonical URLs are blocked by WAFs (Aetna sits behind Incapsula on `www.aetna.com`), but no JSON ships its `source_policy_url` field until a human re-fetches the canonical URL from a non-bot-blocked browser session, runs spot-checks (conservative-therapy thresholds, red-flag taxonomy, ICD code lists, structural OR-semantics), and appends a verification log to `docs/payer_criteria_research.md`. Worked example + log format: PR #6 → `docs/payer_criteria_research.md` → `## Aetna canonical-URL verification log`. Workflow doc: `docs/po_platform_notes.md` → "Payer policy WAFs block automated extraction" entry.
